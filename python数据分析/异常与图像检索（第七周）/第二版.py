...
class ImagePixelDifferenceSimilarity(ImageSimilarity):
	def __init__(self,img1_path,img2_path):
		super().__init__(img1_path,img2_path)
...
class ImageHistogramKendalltauSimilarity(ImageSimilarity):
	def __init__(self,img1_path,img2_path):
		super().__init__(img1_path,img2_path)
...
class ImageHistogramPearsonrSimilarity(ImageSimilarity):
	def __init__(self,img1_path,img2_path):
		super().__init__(img1_path,img2_path)
...
class ImageHistogramSpearmanrSimilarity(ImageSimilarity):
	def __init__(self,img1_path,img2_path):
		super().__init__(img1_path,img2_path)
...
class ImageHashSimilarity(ImageSimilarity):
	def __init__(self,img1_path,img2_path,method='average'):
		super().__init__(img1_path,img2_path)

	def similarity(self,method='average'):
		super().similarity()
		img1 = Image.open(self.img1_path)
		img2 = Image.open(self.img2_path)
		if method == "average":
...
class ImageQuery(object):
	...
	def search_image(self,img_path,Similarity:ImageSimilarity,n,threshold):
		similarity={}
		for k,v in self.images.items():
			p=Similarity(img_path,k)
			if Similarity==ImageHashSimilarity:
				similarity[k]=p.similarity(method='average')
			else:
				similarity[k] = p.similarity()
		...
		if Similarity==ImageHashSimilarity:
			if float(sorted_similarity[1][1])<threshold:
				...
				plt.show()
		if Similarity==ImagePixelDifferenceSimilarity:
			if float(sorted_similarity[-2][1])>threshold:
				...
				for i, j in enumerate(sorted_similarity[:n]):
					...
					plt.imshow(self.images[j[0]])
				plt.show()
		elif Similarity==ImageHistogramKendalltauSimilarity or Similarity==ImageHistogramPearsonrSimilarity or Similarity==ImageHistogramSpearmanrSimilarity:
...
# img.search_image('C:/Users/86137/PycharmProjects/pythonProject/python数据分析/异常与图像检索（第七周）/lfw/Gerhard_Schroeder/Gerhard_Schroeder_0001.jpg',ImageHistogramKendalltauSimilarity,8,0.1)